...
import com.example.shakil.instagram.Adapter.MyPhotosAdapter;
import com.example.shakil.instagram.Common.Common;
import com.example.shakil.instagram.EditProfileActivity;
...
import com.example.shakil.instagram.OptionsActivity;
...
public class ProfileFragment extends Fragment {
    ...
    MyPhotosAdapter savedAdapter;
    List<ImagePostModel> savedImagePostModels;

    List<String> mySavesPhotos;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        adapter = new MyPhotosAdapter(getContext(), imagePostModels);
        ...
        savedImagePostModels = new ArrayList<>();
        savedAdapter = new MyPhotosAdapter(getContext(), savedImagePostModels);
        recycler_saved_photos.setAdapter(savedAdapter);
    ...
    private void mySavedPhotos() {
        mySavesPhotos = new ArrayList<>();

        DatabaseReference reference = FirebaseDatabase.getInstance().getReference(Common.USER_SAVES).child(firebaseUser.getUid());

        reference.addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                for (DataSnapshot snapshot : dataSnapshot.getChildren()){
                    mySavesPhotos.add(snapshot.getKey());
                }
                getSavedPhotos();
            }
    ...
    private void getSavedPhotos() {
        ...
        reference.addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                savedImagePostModels.clear();
                for (DataSnapshot snapshot : dataSnapshot.getChildren()){
                    ImagePostModel imagePostModel = snapshot.getValue(ImagePostModel.class);
                    for (String id : mySavesPhotos){
                        if (imagePostModel.getPostId().equals(id)){
                            savedImagePostModels.add(imagePostModel);
                        }
                    }
                }
    ...
    private void myPhotos() {
        ...
        reference.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                for (DataSnapshot snapshot : dataSnapshot.getChildren()){
                    ImagePostModel imagePostModel = snapshot.getValue(ImagePostModel.class);
                    if (imagePostModel.getPublisher().equals(profileId)){
                        imagePostModels.add(imagePostModel);
                    }
                }
                Collections.reverse(imagePostModels);
    ...
    private void getTotalFollowers() {
        ...
        DatabaseReference followingsReference = FirebaseDatabase.getInstance().getReference()
    ...
    private void addNotifications() {
        DatabaseReference reference = FirebaseDatabase.getInstance().getReference(Common.USER_NOTIFICATION).child(profileId);
